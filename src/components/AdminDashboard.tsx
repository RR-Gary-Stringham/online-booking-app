...
export default function AdminDashboard({
  ...
  const embedCodeSnippet = useMemo(() => {
    ...
    return `<iframe src="${absoluteEmbedUrl}" title="REVREBEL booking calendar" data-rr-booking-embed width="${embedWidth}" height="${embedHeight}" scrolling="no" frameborder="0" style="border: 0;"></iframe>
<script>
(() => {
  ...
  const handleSendTestEmail = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const testMeetingTime = testStart.toISOString();
      const compactUtc = (value: Date) => value.toISOString().replace(/[-:]/g, '').replace('.000', '');
      const testGoogleCalUrl = new URL('https://calendar.google.com/calendar/render');
      testGoogleCalUrl.searchParams.set('action', 'TEMPLATE');
      testGoogleCalUrl.searchParams.set('text', 'REVREBEL Strategic Growth Review');
      testGoogleCalUrl.searchParams.set('dates', `${compactUtc(testStart)}/${compactUtc(testEnd)}`);
      testGoogleCalUrl.searchParams.set('details', 'Test meeting generated by the REVREBEL booking workspace.');
      const testOutlookCalUrl = new URL('https://outlook.office.com/calendar/0/deeplink/compose');
      testOutlookCalUrl.searchParams.set('subject', 'REVREBEL Strategic Growth Review');
      testOutlookCalUrl.searchParams.set('startdt', testStart.toISOString());
      testOutlookCalUrl.searchParams.set('enddt', testEnd.toISOString());
      testOutlookCalUrl.searchParams.set('body', 'Test meeting generated by the REVREBEL booking workspace.');
      const safeAppUrl = publicAppUrl.replace(/\/$/, '');
      const response = await fetch(`${publicAppUrl.replace(/\/$/, '')}/api/send-confirmation`, {
        ...
        body: JSON.stringify({
          ...
          manageUrl: safeAppUrl,
          meetingLink: 'https://meet.google.com/',
          cancelUrl: safeAppUrl,
          rescheduleUrl: `${safeAppUrl}?calendar=revrebel-helpdesk`,
          outlookCalUrl: testOutlookCalUrl.toString(),
          googleCalUrl: testGoogleCalUrl.toString(),
          brevoApiKey,
          brevoSenderEmail,
          brevoSenderName,
